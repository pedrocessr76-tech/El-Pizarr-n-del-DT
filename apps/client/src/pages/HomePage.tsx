  ...
  return (
    <div className="bg-background text-on-background min-h-screen w-full overflow-hidden pitch-gradient relative flex flex-col justify-start md:justify-center items-center px-gutter py-lg md:py-0 transition-all duration-500">
      ...
      <div className="relative z-10 w-full flex flex-col gap-4 md:hidden" id="main-content-mobile">
        ...
        {/* Acciones principales: lista agrupada profesional */}
        <div className="w-full rounded-2xl border border-white/10 overflow-hidden shadow-lg shadow-black/25 bg-surface-container-high">
          {/* Jugar Partido (destacado) */}
          <button
            onClick={() => onNavigate('builder')}
            className="w-full flex items-center gap-3 px-4 h-16 bg-primary-container text-left active:brightness-110 transition-all"
          >
            <span className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-[24px] text-primary">sports_soccer</span>
            </span>
            <span className="flex-1 min-w-0">
              <span className="block font-headline-sm text-[15px] font-black uppercase tracking-wide truncate text-[#dff2e6]">Jugar Partido</span>
              <span className="block font-label-md text-[11px] truncate text-[#86af99] mt-0.5">Enfrentate en el torneo Élite</span>
            </span>
            <span className="material-symbols-outlined text-[20px] text-secondary shrink-0">arrow_forward</span>
          </button>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 h-16 text-left active:bg-surface-variant transition-colors"
          >
            <span className="w-10 h-10 rounded-lg bg-secondary/20 flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-[24px] text-secondary">history</span>
            </span>
            <span className="flex-1 min-w-0">
              <span className="block font-headline-sm text-[15px] font-bold uppercase tracking-wide truncate text-on-surface">Ver Historial</span>
              <span className="block font-label-md text-[11px] truncate text-on-surface-variant mt-0.5">Revisa tus últimos partidos</span>
            </span>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 h-16 text-left active:bg-surface-variant transition-colors"
          >
            <span className="w-10 h-10 rounded-lg bg-tertiary/20 flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-[24px] text-tertiary">style</span>
            </span>
            <span className="flex-1 min-w-0">
              <span className="block font-headline-sm text-[15px] font-bold uppercase tracking-wide truncate text-on-surface">Ver Catálogo de Cartas</span>
              <span className="block font-label-md text-[11px] truncate text-on-surface-variant mt-0.5">Explora tu colección de jugadores</span>
            </span>
            <span className="material-symbols-outlined text-[20px] text-on-surface-variant shrink-0">chevron_right</span>
          </button>
        </div>
      ...
      <div className="relative z-10 w-full hidden md:flex flex-col justify-center items-center" id="main-content">
        ...
        {/* Main Actions Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full max-w-5xl mt-lg">
          {/* JUGAR Card */}
          <button
            onClick={() => onNavigate('builder')}
